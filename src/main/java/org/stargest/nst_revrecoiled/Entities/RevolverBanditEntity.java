...
public class RevolverBanditEntity extends RaiderEntity {
    ...
    protected void equipRevolver() {
        ...
        if (held.isEmpty() || !(held.getItem() instanceof BaseRevolverItem)) {
            Item configItem = Registries.ITEM.get(new Identifier(ModConfig.get().bandit.equippedItem));
    ...
    protected void tickWeapon() {
        ...
        if (pendingFireTicks >= 0) {
            if (pendingFireTicks == 0) {
                Item ammoItem = Registries.ITEM.get(new Identifier(ModConfig.get().bandit.ammoItem));
        ...
        if (charged && canAttack) {
            ...
            if (nbt != null) nbt.remove("ChargedBullet");
        ...
        if (!charged && canAttack) {
            ...
            if (chargingTicks >= revolver.getChargeTimeTicks()) {
                ...
                new ItemStack(ammoItem).writeNbt(bulletNbt);
                stack.getOrCreateNbt().put("ChargedBullet", bulletNbt);

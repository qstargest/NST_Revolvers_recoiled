...
public class RevolverBanditEntity extends Raider {
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(CHARGE_PROGRESS, 0.0f);
        entityData.define(IS_CHARGING, false);
    }
    ...
    public @Nullable SpawnGroupData finalizeSpawn(@NotNull ServerLevelAccessor level,
                                                  ...
                                                  @Nullable SpawnGroupData spawnGroupData, @Nullable CompoundTag nbt) {
        SpawnGroupData data = super.finalizeSpawn(level, difficulty, mobSpawnType, spawnGroupData, nbt);
    ...
    protected void equipRevolver() {
        ...
        if (held.isEmpty() || !(held.getItem() instanceof BaseRevolverItem)) {
            ResourceLocation configLoc = new ResourceLocation(ModConfig.get().bandit.equippedItem);
            Item configItem = BuiltInRegistries.ITEM.get(configLoc);
    ...
    protected void tickWeapon() {
        ...
        if (pendingFireTicks >= 0) {
            if (pendingFireTicks == 0) {
                ResourceLocation ammoLocation = new ResourceLocation(ModConfig.get().bandit.ammoItem);
        ...
        if (charged && canAttack) {
            ...
            revolver.writeProjectilesToNbt(stack, Collections.emptyList());
        ...
        if (!charged && canAttack) {
            ...
            if (chargingTicks >= revolver.getChargeTimeTicks()) {
                ...
                revolver.writeProjectilesToNbt(stack, List.of(new ItemStack(ammoItem)));
